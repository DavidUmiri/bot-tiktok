...
async def descargar_video(update: Update, url: str):
    ...
    ydl_opts = {
        ...
        "outtmpl": str(DOWNLOADS_DIR / f"{random_filename}.%(ext)s"),
        "quiet": True,
    }
            ...
            with cleanup_file(file_path):
                if file_path.endswith((".mp4", ".webm")):
                    await update.message.reply_video(
                        ...
                        filename=f"video_{random_filename}.mp4",
                    )
                else:
                    await update.message.reply_audio(
                        audio=open(file_path, "rb"),
                        caption="Aquí tienes tu audio.",
                        filename=f"audio_{random_filename}.m4a",
                    )
    ...
    try:
        ...
        try:
            ...
            await update.message.reply_text(
                "No se encontró el contenedor del reproductor de audio."
            )
            return
        ...
        try:
            ...
            await update.message.reply_text(
                "No se encontró el elemento de video en el contenedor."
            )
            return
        ...
        if not audio_url:
            await update.message.reply_text(
                "El elemento de video no tiene atributo src."
            )
            return
        ...
        if response.status_code == 200:
            random_filename = f"{uuid.uuid4().hex}"
            file_path = DOWNLOADS_DIR / f"{random_filename}.m4a"
            ...
            await update.message.reply_text(
                "No se pudo descargar el audio desde el enlace extraído."
            )
    except Exception as e:
...
async def descargar_fotos(update: Update, url: str):
    ...
    try:
        ...
        # Recolectar URLs y loguear para depuración
        image_urls = []
        seen_urls = set()  # Conjunto para tracking de URLs únicas

        for img in images:
            src = img.get_attribute("src")
            if src:
                logger.info(f"Found image URL: {src}")
                # Solo añadir si no hemos visto esta URL antes
        ...
        if image_urls:
            ...
            for i in range(0, len(image_urls), 10):
                media_group = [
                    InputMediaPhoto(media=url) for url in image_urls[i : i + 10]
                ]
                await update.message.reply_media_group(media=media_group)
            if len(image_urls) > 10:
                await update.message.reply_text(
                    f"Se enviaron {len(image_urls)} imágenes."
                )
        else:
            await update.message.reply_text(
                "No se encontraron imágenes en este TikTok."
            )
